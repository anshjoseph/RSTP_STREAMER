...
import subprocess
class RTSP:
    def __init__(self,rtsp_url:str,name:str) -> None:
    ...
    def kill(self)->bool:
        if self.processStart:
            ...
            return False
    def getURL(self)->str:
        return f"http://127.0.0.1:4123/Stream/{self.name}/playlist.m3u8"
    def clean(self)->None:
...
class RTSP_MANAGER:
    ...
    def add(self,rtsp_url:str,name:str):
        if self.rtsp.get(name) == None:
            self.rtsp[name] = RTSP(rtsp_url,name)
...
if __name__ == "__main__":
    ...
    rtsp_manager.add("rtsp://rtspstream:b6ab3b161afed7213fe35a7b5564a179@zephyr.rtsp.stream/movie","bunny")